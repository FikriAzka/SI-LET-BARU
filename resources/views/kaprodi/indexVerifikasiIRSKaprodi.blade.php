...
@section('js')
    ...
    <script>
                        ...
                        if (response.success) {
                            ...
                            if (irsData.length > 0) {
                                ...
                                for (var nim in groupedData) {
                                    ...
                                    tableHtml += '<tr>';
                                    tableHtml += '<td class="border border-gray-300 px-4 py-2">' + nim +
                                        '</td>';
                                    tableHtml += '<td class="border border-gray-300 px-4 py-2">' +
                                        finalStatus + '</td>';
                                    ...
                                    if (finalStatus === 'pending') {
                                        ...
                                        tableHtml +=
                                            '<button class="cancel-btn bg-red-500 text-white px-4 py-2 rounded" data-nim="' +
                                            nim + '">Batalkan</button>';
                                    }
                                    tableHtml += '</td>';
        ...
        // Fungsi baru untuk membatalkan persetujuan IRS
        function cancelIrs(nim) {
            $.ajax({
                url: '/cancel-irs', // Tambahkan endpoint baru di backend untuk membatalkan IRS
                type: 'POST',
                data: {
                    nim: nim,
                    _token: '{{ csrf_token() }}'
                ...
                success: function(response) {
                    if (response.success) {
                        alert('Persetujuan IRS mahasiswa berhasil dibatalkan.');
                        $('#angkatan').trigger('change'); // Refresh data tabel
                    } else {
                        alert('Gagal membatalkan persetujuan IRS mahasiswa.');
                    }
                },
